...
export const StatsTab: React.FC = () => {
  ...
  const { loading, error, refetch } = useQuery(COMPLETED_LIST_QUERY, {
    variables: { userId },
    skip: !userId
  })

  // Fetch only if not cached or dirty
  useEffect(() => {
    if (!userId) return
  ...
  const entries = statsList ?? []

  // Score distribution
  const allScores = useMemo(() => {
    const scores: Record<number, boolean> = {}
    entries.forEach((e: any) => e.score && (scores[e.score] = true))
    return Object.keys(scores).map(Number).sort((a, b) => a - b)
  }, [entries])

  const years = useMemo(() => {
    const set = new Set<number>()
    entries.forEach((e: any) => e.media.seasonYear && set.add(e.media.seasonYear))
  ...
  // mean score
  const meanScore = totalWatched
    ? filtered.reduce((sum, e: any) => sum + (e.score || 0), 0) / totalWatched
    : 0

  // Score distribution
  const scoreData = useMemo(() => {
    const counts: Record<number, number> = {}
    filtered.forEach((e: any) => e.score && (counts[e.score] = (counts[e.score] || 0) + 1))
